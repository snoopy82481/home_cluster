...
5. **Secrets** — does the app need secret environment variables? Ask for every exact variable name and value that belongs in the Secret; never guess names or values. If secrets are needed, create `app/secret.sops.yaml` as a SOPS-encrypted Kubernetes Secret.
...
└── app/
    ...
    ├── ocirepository.yaml
    ├── helmrelease.yaml
    ├── secret.sops.yaml         # only if secrets
...
### app/secret.sops.yaml (only if secrets)

```yaml
---
# yaml-language-server: $schema=https://kubernetes-schemas.pages.dev/v1.34.0/secret-v1.json
apiVersion: v1
kind: Secret
metadata:
  name: <app>-secret
...
Use the exact variables gathered in Step 1 as `stringData` keys. Encrypt the completed manifest with the repository's SOPS configuration before handing it off; do not leave plaintext values in the tracked file. `metadata.name` is `<app>-secret`, matching the `envFrom.secretRef` in the HelmRelease. If the variables or values were not provided, stop and ask rather than adding placeholders.
...
- **Using ExternalSecret or 1Password** — this repository stores workload secrets as encrypted `secret.sops.yaml` Kubernetes Secret manifests.
